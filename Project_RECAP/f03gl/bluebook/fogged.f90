...
subroutine SetupRC
   ...
   call glEnable(GL_FOG)                  ! Turn Fog on
   call glFogfv(GL_FOG_COLOR, LowLight)   ! Set fog color to match background
   call glFogf(GL_FOG_START, 10.0)        ! How far away does the fog start
   call glFogf(GL_FOG_END, 20.0)          ! How far away does the fog stop
   call glFogi(GL_FOG_MODE, GL_LINEAR)    ! Which fog equation do I use?
...
subroutine RenderScene() bind(C)
   ...
   call glClear(ior(GL_COLOR_BUFFER_BIT, &
        ior(GL_DEPTH_BUFFER_BIT,GL_ACCUM_BUFFER_BIT)))
