module glutdino
  ...
  private :: extrudeSolidFrompolygon
  ...
  logical, private, save :: lightZeroSwitch = .true.
  logical, private, save :: lightOneSwitch = .true.
  ...
  subroutine extrudeSolidFrompolygon(data, dataSize, &
    ...
    do i=1,fcount
      ...
      vertex(3) = 0.0
      !this version is not accepted by gfortran 4.4:
  ...
  subroutine redraw() bind(C)
    ...
    if( newmodel )then
      x=cos(pi*angle%x/180.0)
      z=sin(pi*angle%x/180.0)
      call glpopmatrix()
      call glpushmatrix()
      call glrotatef(angle%x, 0.0, 1.0, 0.0)
      call glrotatef(angle%y, x, 0.0, z)
