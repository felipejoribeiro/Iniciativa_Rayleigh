...
\documentclass[twocolumn,10pt]{registro}
...
\title{Bidimensional flow in a cavity. A study about the parametrization of the turbulent Prandtl number and the Cebeci constant}

%%% first author
\author{Felipe Jos\'{e} Oliveira Ribeiro
    \affiliation{
	Undergraduate\\
	Department of Mechanical Engineering\\
	University of Uberl\^{a}ndia\\
	Uberl\^{a}ndia, Jo\~{a}o Naves de \'{A}vila 2121\\
    Email: feliperibeiro.ufu@gmail.com
    }	
}
...
\author{Aristeu da Silveira Neto %\thanks{Orienta}
    \affiliation{ 
    Professor\\
	Department of Mechanical Engineering\\
	University of Uberl\^{a}ndia\\
	Uberl\^{a}ndia, Jo\~{a}o Naves de \'{A}vila 2121\\
	Email: aristeus@ufu.br
    }
}
...
\begin{abstract}
{\it 
	Numerical procedures are common in Computational Fluids Dynamics. Turbulence results in nonlinear behavior with complex partial differential equations generally impossible to solve with pure analytical methodologies. There is a big number of numerical methods to be used in such problems, with simplifications like the Boussinesq hypothesis that transforms the Navier-Stokes equations in more feasible mathematical problems. With these reduced systems of finite-difference equations, implicit numerical methodologies can be applied. In the Cavity problem, a state equation is used to change the specific mass of the fluid in function of the temperature. From this, depending on the boundary conditions, in a rectangular domain, it is possible to observe natural convection. The present work aim in study such phenomenon, varying the turbulent Prandtl number and the constant of Cebeci for better results. For solving the implicit method faster, OPENmpi will be performed with FORTRAN. For Graphical representation OPENGL will be used for the same language. With the simultaneous usage of such tools it is possible to create a real time accurate reactive cavity simulation, with complex and transient Boundary conditions.  
}
\end{abstract}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{nomenclature}
\entry{$x$, $y$, $z$}{Inertial reference coordinates.}
\entry{$u$, $v$, $w$}{Velocities in each Inertial reference direction.}
\entry{$T$}{Temperature.}
\entry{$t$}{Time.}
\entry{$\mu$}{Viscosity.}
\entry{$\nu$}{Kinetic viscosity.}
\entry{$p$}{Pressure.}
\entry{$\rho$}{Specific mass.}
\entry{$Re$}{Reynolds number.}
\entry{$Pr$}{Prandtl number.}
\entry{$Pr_t$}{Turbulent Prandtl number.}
\entry{$[~]_\tau$}{Wall variable.}
\entry{$\overline{[~]}$}{Mean value.}
\entry{$[~]^\prime$}{Fluctuation.}
\end{nomenclature}
...
\begin{figure} 
\centerline{\psfig{figure=images/FMANU_MD_05_1107_11,width=3.34in}}
...
%%%%%%%%%%%%%%%% begin figure %%%%%%%%%%%%%%%%%%%
\begin{figure} 
\centerline{\psfig{figure=images/FMANU_MD_05_1272_5,width=3.34in}}
...
\centerline{\psfig{figure=images/FMANU_MD_04_1274_13,width=3.25in}}
...
\begin{figure*} 
\centerline{\psfig{figure=images/FMANU_MD_04_1274_13,width=6.85in}}
...
\bibliographystyle{registro}

% Here's where you specify the bibliography database file.
% The full file name of the bibliography database for this
% article is asme2e.bib. The name for your database is up
% to you.
\bibliography{registro}
