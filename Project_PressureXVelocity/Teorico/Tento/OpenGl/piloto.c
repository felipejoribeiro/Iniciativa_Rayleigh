...
int main(int argc, char **argv) {
    glutInit(&argc, argv);                           //initiate glut instance
    glutInitWindowSize(WIDTH, HEIGHT);               //Determine window geometry
    glutInitWindowPosition(100, 100);                //Determine window position of apearance
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGBA);    //Determine dysplay mode
    glutCreateWindow(argv[0]);                       //Comand for window creation

    //Determination of protocolls
    init();                                          //Function over there, configuring the window
    glutDisplayFunc(display);
    glutIdleFunc(idle);
    glutMouseFunc(mouse);
    atexit(deinit);                                  //Function called after window close

    glutMainLoop();                                  //Initiate Opengl cicle
    return EXIT_SUCCESS;
}
